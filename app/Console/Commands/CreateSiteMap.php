...
class CreateSiteMap extends Command
{
    ...
    public function handle()
    {
        ...
        // tạo sitemap tap phim
        $movie_ep = Movie::with('episode')->orderBy('id', 'DESC')->get();
        $episode = Episode::all();
        foreach ($movie_ep as $mov_ep) {
            foreach ($episode as $ep) {
                if ($mov_ep->id == $ep->movie_id) {$sitemap->add(env('APP_URL') . "/xem-phim/{$mov_ep->slug}/tap-{$ep->episode}",Carbon::now('Asia/Ho_Chi_Minh'),'0,6','daily');
                }
            }
        }
           // tạo sitemap năm phim
